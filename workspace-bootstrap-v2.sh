...
# --- Dev services script ---
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
if [ ! -f "$SCRIPT_DIR/dev-services.sh" ]; then
  cat >"$SCRIPT_DIR/dev-services.sh" <<'DEVEOF'
#!/bin/bash
# ============================================================
# dev-services.sh — Launch 4 workspace services in tmux panes
#
# Usage: bash ~/dotfiles/dev-services.sh
# Or use the fish alias: services
#
# If the session already exists, attaches to it.
# To kill the session: tmux kill-session -t services
# ============================================================
...
# ============================================================
# CONFIGURE YOUR 4 SERVICE COMMANDS HERE
# ============================================================
CMD1="echo 'Service 1 — replace with your command'"
CMD2="echo 'Service 2 — replace with your command'"
CMD3="echo 'Service 3 — replace with your command'"
CMD4="echo 'Service 4 — replace with your command'"

# Example commands (uncomment and modify):
# CMD1="pnpm run dev:api"
# CMD2="pnpm run dev:app"
# CMD3="pnpm run dev:worker"
# CMD4="pnpm run dev:watch"
...
# Rebalance panes to be equal width
tmux select-layout -t "$SESSION" even-horizontal

# Send commands to each pane
tmux send-keys -t "$SESSION:1.1" "$CMD1" C-m
tmux send-keys -t "$SESSION:1.2" "$CMD2" C-m
tmux send-keys -t "$SESSION:1.3" "$CMD3" C-m
tmux send-keys -t "$SESSION:1.4" "$CMD4" C-m

# Attach to the session
tmux attach-session -t "$SESSION"
DEVEOF
  chmod +x "$SCRIPT_DIR/dev-services.sh"
  echo "  Dev services script created — edit commands in ~/dotfiles/dev-services.sh"
else
  echo "  ✓ Dev services script already exists"
fi
